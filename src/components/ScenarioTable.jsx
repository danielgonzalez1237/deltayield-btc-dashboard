  ...
  return (
    <div className="bg-[#111119] border border-[#1f1f30] rounded-2xl p-5">
      <div className="mb-5">
        ...
        <p className="text-[10px] text-[#4e4e66] mt-0.5">All 8 configurations ranked by CAGR</p>
      </div>
        ...
        <table className="w-full text-[11px] min-w-[900px]">
          <thead>
            <tr className="text-[#4e4e66] text-[10px] uppercase tracking-[0.08em]">
              ...
              <th className="text-right py-2 px-2 font-semibold">Final BTC</th>
          ...
          <tbody>
            {sorted.map((s, i) => {
              ...
              return (
                <tr
                  key={s.key}
                  className={`border-t border-[#1f1f30] transition-colors hover:bg-[rgba(255,255,255,0.015)] ${
                    best ? 'bg-[rgba(247,147,26,0.03)]' : ''
                  }`}
                >
                  ...
                  <td className="py-3 px-2 font-medium text-[#eaeaf2] whitespace-nowrap">
                    {s.key}
                  </td>
                  <td className="py-3 px-2 text-right font-mono tabular-nums text-[#f7931a]">{m.finalBtc.toFixed(4)}</td>
                  ...
                  <td className="py-3 px-2 text-right font-mono tabular-nums text-[#60a5fa]">{m.cagrDdRatio.toFixed(1)}</td>
                  <td className="py-3 px-2 text-right font-mono tabular-nums text-[#34d399]">+{c.fees.toFixed(4)}</td>
                  <td className={`py-3 px-2 text-right font-mono tabular-nums ${c.hedge > 0 ? 'text-[#34d399]' : 'text-[#4e4e66]'}`}>
                    {c.hedge > 0 ? `+${c.hedge.toFixed(4)}` : c.hedge.toFixed(4)}
                  </td>
                  <td className="py-3 px-2 text-right font-mono tabular-nums text-[#f87171]">-{c.il.toFixed(4)}</td>
                  <td className={`py-3 px-2 text-right font-mono tabular-nums font-semibold ${c.net >= 0 ? 'text-[#34d399]' : 'text-[#f87171]'}`}>
                    {c.net >= 0 ? '+' : ''}{c.net.toFixed(4)}
                  </td>
