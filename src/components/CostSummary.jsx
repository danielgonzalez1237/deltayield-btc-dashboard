export default function CostSummary({ costs, metrics }) {
  const { fees, hedge, il, gas, slippage, swapFees, perpFees, net } = costs;
  const startBtc = 1.0;

  const rows = [
    { label: 'Fees Earned', btc: fees, icon: '+', type: 'income' },
    { label: 'Hedge P&L', btc: hedge, icon: '+', type: 'income' },
    { label: 'Impermanent Loss', btc: -il, icon: '-', type: 'cost' },
    { label: 'Gas Costs', btc: -gas, icon: '-', type: 'cost' },
    { label: 'Slippage', btc: -slippage, icon: '-', type: 'cost' },
    { label: 'Swap Fees', btc: -swapFees, icon: '-', type: 'cost' },
    { label: 'Perp Fees', btc: -perpFees, icon: '-', type: 'cost' },
  ];
  ...
  return (
    <div className="bg-[#111119] border border-[#1f1f30] rounded-2xl p-5 h-full flex flex-col">
      ...
      <p className="text-[10px] text-[#4e4e66] mb-4">All values in BTC</p>
          ...
          return (
            <div key={row.label} className="group py-2 px-3 rounded-lg hover:bg-[rgba(255,255,255,0.02)] transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] text-[#7a7a96] group-hover:text-[#eaeaf2] transition-colors">
                  {row.label}
                </span>
                <div className="flex items-center gap-3">
                  <span className={`text-[11px] font-mono tabular-nums ${isPositive ? 'text-[#34d399]' : 'text-[#f87171]'}`}>
                    {isPositive ? '+' : ''}{row.btc.toFixed(6)}
                  </span>
              ...
              <div className="h-[3px] bg-[#1f1f30] rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${isPositive ? 'bg-[#34d399]' : 'bg-[#f87171]'}`}
      ...
      <div className={`mt-3 pt-3 border-t border-[#1f1f30] px-3 py-2 rounded-xl ${
        net >= 0
          ? 'bg-[rgba(52,211,153,0.05)] border-t-[rgba(52,211,153,0.15)]'
          : 'bg-[rgba(248,113,113,0.05)] border-t-[rgba(248,113,113,0.15)]'
      }`}>
