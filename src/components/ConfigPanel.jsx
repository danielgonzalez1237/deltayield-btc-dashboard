...
export default function ConfigPanel({
  ...
  return (
    <div className="bg-[#111119] border border-[#1f1f30] rounded-2xl p-5 mb-5">
      <div className="flex flex-wrap gap-5 items-end">
        ...
        <PillToggle
          ...
          options={[
            ...
            { value: 'sma', label: 'SMA Timing' },
          ]}
        />
        <PillToggle
          label="Hedge"
          value={hedge}
          onChange={v => setHedge(v)}
          options={[
            { value: true, label: 'With Hedge' },
            { value: false, label: 'No Hedge' },
          ]}
        />
        <PillToggle
          label="Off Mode"
          ...
          onChange={setOffMode}
          options={[
            { value: 'A', label: 'A: WBTC' },
            { value: 'B', label: 'B: Conserve' },
          ]}
        />
        <PillToggle
          label="Benchmark"
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2.5 bg-[#0c0c14] border border-[#1f1f30] rounded-lg px-3 py-[5px]">
            <input
              ...
              min="0"
              max="50"
              value={slippage * 10000}
              onChange={e => setSlippage(parseInt(e.target.value) / 10000)}
              className="w-16"
            />
      ...
      {/* Gas simulator expanded */}
      {gasOpen && (
        <div className="mt-4 pt-4 border-t border-[#1f1f30] space-y-3">
          <div className="flex items-center gap-4">
            <span className="text-[11px] text-[#7a7a96]">Gas cost per tx:</span>
            <input
              type="range"
              min="0"
              max="500"
          ...
          <div className="flex gap-3 text-[10px] text-[#4e4e66]">
            <span>2021: $3.00</span>
            <span>2022: $1.50</span>
            <span>2023: $0.84</span>
            <span>2024 Q1: $1.50</span>
            <span>2024+: $0.03</span>
          </div>
        </div>
      )}
